...
ただし、**3軸を物理分割しても同じ target path は共有しない**ことを前提条件として確定する。

- `セット = 複数 repo の束` ではなく、`構成 = 複数の独立軸（レイヤ）の組み合わせ` として扱う。
- 3軸は次の意味で固定する。
  - `ssot-core`: 技術・言語・AI 別入口・共通入口ファイル
  - `ssot-schema`: 開発プロセス・設計運用ルール
  - `ssot-policies`: 展開先制約・セキュリティ・禁止事項などの環境ポリシー
- 物理分割の主基準は引き続き `path / 権限境界` とし、同じ target path を複数 repo で共有しない。
- `ssot-core` は `.github/copilot-instructions.md`、`.github/copilot/00-index.md`、`AGENTS.md` などの入口・共通ファイルの最終責任を持つ。
- `ssot-schema` は `.github/instructions/**` のような運用ルール系ファイル、`ssot-policies` は `40-testing-strategy.md` / `50-security.md` のようなポリシー系ファイルを、それぞれ**専有 path**として持つ。
- 軸の組み合わせは DESIGN 時に決め、`ssot-sync-controller` は merge せず catalog path をそのまま target repo へ配る。

要するに、**3軸分割は採用するが、配布方式は変えず、担当 path を分けて衝突を起こさない**という整理である。
...
役割差による分割は **補助的表現（ディレクトリ・metadata・軸名）に限定し、主基準にはしない**。

#### 理由

1. 後勝ちは**例外時の逃げ道**であり、平常運用で衝突を前提にしないため
   - 現行仕様は後勝ちを持つが、通常設計では衝突しない catalog を作る前提である。
   - したがって、役割だけで repo を分けて同一 path に配る構造は主基準にしにくい。

2. 役割単位だけでは最終配置 path と 1 対 1 に対応しないため
   - `skills-core` / `skills-provider` のような役割は論理分類であり、どこへ配置されるかを直接表さない。
   - そのまま主基準にすると、同一 path を複数 repo が触りやすくなる。

3. `path / 権限境界` は物理配置と CI / 権限管理に直結するため
   - 「どこに置くか」と「どの repo が責任を持つか」を一致させやすい。
   - `allowed-paths.yml` や review 境界とも整合しやすい。
...
- 3軸や role 名そのものを否定するわけではない。
- ただし、**repo を増やす判断の入口**は `path / 権限境界` に置く。
- role / 軸は、その上で「何の責務か」を説明する補助ラベルとして使う。
...
① `skill-catalog` などの metadata repo に「推奨 version tuple」を持たせ、DESIGN 時に Planner が参照する

- ここでいう version tuple とは、`ssot-core` / `skills-*` / `mcp-*` / `infra-runtime` などをどの ref の組み合わせで使うかを並べた推奨表を指す。
- 例: `skill-catalog` 内の YAML 台帳に、`ssot-core: v1.2.3`、`skills-core: v2.0.1`、`mcp-tools: v1.4.0`、`infra-runtime: v0.9.0` のような対応表として記録する。
- `skill-catalog` は設計時の判断材料としてのみ使う。
- target repo / `ssot-sync-controller` / executor は実行時に参照しない。
- 最終的な採用値は DESIGN Issue に固定し、実行系はその固定値だけを使う。
...
| 選択肢 | 内容 | 特徴 |
| --- | --- | --- |
| ① `ssot-schema` / `ssot-policies` は担当ファイルを丸ごと差し替える単位として扱う | set ごとに完成済みファイルを持つ | controller が単純なままで済む |
| ② 1ファイルの一部だけを軸ごとに差し込む | 章や節だけを切り替える | merge 仕様が増え、複雑になる |
| ③ `00-index.md` などの基底ファイルは `ssot-core` 固定にし、schema/policies は自分の担当 path にある周辺ファイルだけを丸ごと差し替える（推奨） | 変更対象を限定しつつ merge を避ける | 安全で、現行仕様とも整合しやすい |

#### 推奨
③ `00-index.md` などの基底ファイルは `ssot-core` 固定にし、schema/policies は自分の担当 path にある周辺ファイルだけを丸ごと差し替える

##### 理由
- `00-index.md` や AI 別入口のような共通ファイルは `ssot-core` に寄せた方が責任境界を明確にできる。
- `ssot-schema` / `ssot-policies` は、自分の担当 path にあるファイルを**丸ごと差し替える**だけにすると、現行仕様の「catalog path をそのままコピーする」を崩さずに済む。
- 一部差し込みを避ければ、質問10の専有 path ルールとも矛盾しにくい。

### 回答

③ `00-index.md` などの基底ファイルは `ssot-core` 固定にし、schema/policies は自分の担当 path にある周辺ファイルだけを丸ごと差し替える

- `ssot-core`: `.github/copilot-instructions.md`、`.github/copilot/00-index.md`、AI 別入口ファイル
- `ssot-schema`: `.github/instructions/**` などの運用ルール系
- `ssot-policies`: `40-testing-strategy.md` / `50-security.md` などのポリシー系
- 1 ファイルの一部差し込みは行わず、担当ファイルは各 repo が完成形を持つ
...
① 共通ファイルと衝突しやすい path だけ先に固定し、残りは段階的に決める

- 最初から全 path を確定する必要はない。
- ただし `.github/` 配下や入口ファイルのように衝突しやすい場所は、先に担当 repo を固定する。
- 初期ドラフトは表形式で別ファイルへまとめ、段階的に更新していく。
...
- version が合っていても、`ssot-core` / `ssot-schema` / `ssot-policies` の前提がかみ合うかは Planner が DESIGN 時に確認する。
- ここで見るのは番号ではなく、技術前提・運用前提・制約前提が同時に成立するかどうかである。
- 確認結果は DESIGN Issue に明記し、IMPLEMENT / controller はその判断済み内容を前提に進める。
- `ref` 固定の扱い自体は質問18で確定する。
...
## 質問15〜17への回答を踏まえて、まだ残った追加質問

質問15〜17への今回の回答追記を前提にもう一度読み直したところ、**新しく 2 つだけ追加で確認した方がよい点**が残った。  
この 2 点が決まれば、いったん追加質問は止めて DESIGN へ渡しやすくなる。

## 質問18: `main` ブランチを最新として見る運用にするなら、仕様の `ref` 固定とどう合わせるのか

簡潔に言うと、**「いつも最新の `main` ブランチを見る」のか、「決まった tag / branch / commit を使う」のか**をそろえる質問である。

### 未確定事項

#### 問題
Issue コメントや検討メモでは「各 repo の `main` を最新として見る」運用案も出ている。  
一方で現行仕様の `docs/ssot-distribution-system-spec-final-2026-04-04.md` 7.2 では、`ssot-bot.yml` の `ref` は固定 version（tag / branch / commit）と定義されている。  
このままだと、「target repo は固定 ref を書くのか」「毎回 `main` ブランチを最新として見るのか」で整合性が取れていない。

#### 何が何に影響するか
この判断は `ssot-bot.yml` の書き方、rollback 方法、差分の再現性に影響する。  
固定 ref なら、どの状態を使ったかを後で追いやすい。  
一方、main 追従にすると運用は単純に見えるが、いつ内容が変わったかを追いにくくなる。

### 選択肢

| 選択肢 | 内容 | 特徴 |
| --- | --- | --- |
| ① 現行仕様どおり `ref` 固定を維持する（推奨） | tag / branch / commit を明記する | 再現しやすい |
| ② `main` 固定を正式仕様にする | 常に最新 main を参照する | 運用は単純だが変化を追いにくい |
| ③ SSOT だけ `main`、他は `ref` 固定にする | ドメインごとに分ける | ルールが増える |

#### 推奨
① 現行仕様どおり `ref` 固定を維持する

##### 理由
- すでに現行仕様が固定 ref を前提にしている。
- rollback や差分確認の説明を変えずに済む。
- 「最新を追う」運用を入れるなら、まず仕様変更の判断が必要になる。

### 回答

① 現行仕様どおり `ref` 固定を維持する（推奨）

- `main` を最新として追いたい場合でも、そのまま実行仕様にはせず、DESIGN 時に採用する `ref` へ落とし込んで target repo に固定する。
- 実行系は常に固定された `ref` だけを見て再現性を保つ。

---

## 質問19: 担当 path を最初に全部決めないなら、何を最小限だけ先に決めるのか

簡潔に言うと、**全部のファイルを最初に決めないとしても、ぶつからないための最低限の約束は何か**を決める質問である。

### 未確定事項

#### 問題
見出し「## 質問15: 12個の repo のうち、どの repo がどのファイルを担当するのか」の回答では、「最初に全部決めるのは厳密すぎるので、少しずつ決めたい」とされている。  
この考え方自体は自然だが、現行仕様では `docs/ssot-distribution-system-spec-final-2026-04-04.md` 6章, 17.1, 18.1 にあるように、出力先構造が異なる系統は物理 repo を分ける前提がある。  
そのため、少しずつ決める場合でも、「最初に何も決めない」は難しく、少なくとも衝突しやすい path や共通ファイルの責任範囲だけは先に固定する必要がある。

#### 何が何に影響するか
この判断は repo 追加の順番、path 衝突の防止、設計開始のしやすさに影響する。  
最低限の約束がないと、後から repo を増やすたびに担当範囲がぶれやすい。  
逆に最小限だけ先に決めれば、「全部は決めないが、危ない場所だけは先に守る」という進め方ができる。

### 選択肢

| 選択肢 | 内容 | 特徴 |
| --- | --- | --- |
| ① 共通ファイルと衝突しやすい path だけ先に固定する（推奨） | `.github/` など重要 path だけ先に担当を決める | 段階導入しやすい |
| ② repo ごとに順番に決め、事前固定はしない | その都度決める | 後からぶつかりやすい |
| ③ 最初に全 path を棚卸ししてから始める | 完全に決めてから進める | 安全だが重い |

#### 推奨
① 共通ファイルと衝突しやすい path だけ先に固定する

##### 理由
- 質問15の「少しずつ決めたい」と、現行仕様の「物理分割ルール」を両立しやすい。
- いきなり全部を表にしなくても、危険な場所だけは最初に守れる。
- DESIGN では「どこまでを先に固定するか」だけをまず決めればよくなる。

### 回答

① 共通ファイルと衝突しやすい path だけ先に固定する

- 初期ドラフトは表形式で別ファイルにまとめる。
- そのドラフトを起点に、残りの path は repo 追加時に段階的に決める。

---

## 質問20: 今回の整理後も、質問・矛盾点は本当に無いのか

かんたんに言うと、**この文書を DESIGN に渡す前に、まだ説明不足や回答の食い違いが残っていないかを最後に確認する質問**である。

### 未確定事項

#### 問題
今回の読み直しでは、少なくとも次の 2 種類が混ざっていた。

- **推奨案と回答が違うだけの箇所**  
  これは「推奨」と「最終回答」を分けている以上、必ずしも矛盾ではない。
- **前提条件が書かれておらず、回答だけ読むと別の章とぶつかる箇所**  
  これは実際に矛盾として見えるため、補足や修正が必要である。

特に質問1、質問2、質問11、質問16、質問18は、前提条件や参照先が不足していると読み手が誤解しやすかった。

#### 何が何に影響するか
この確認は DESIGN へ渡す資料の信頼性に影響する。  
内部矛盾が残ったままだと、DESIGN 側が「どの回答を前提にしてよいか」を判断しにくい。  
逆に、回答同士の接続条件をここで明文化すれば、残るのは DESIGN で決めるべき論点だけに絞りやすくなる。

### 選択肢

| 選択肢 | 内容 | 特徴 |
| --- | --- | --- |
| ① 内部矛盾は今回の修正で解消し、残るのは DESIGN で確認すべき論点だけだと明記する（推奨） | 「修正済みの矛盾」と「これから決める論点」を分ける | 読み手が迷いにくい |
| ② まだ回答同士の矛盾が残っている前提で DESIGN に渡す | 後工程で吸収する | 誤読と手戻りが増えやすい |
| ③ 追加質問はもう不要とみなし、この文書の確認を打ち切る | 作業は早い | 不整合を持ち込みやすい |

#### 推奨
① 内部矛盾は今回の修正で解消し、残るのは DESIGN で確認すべき論点だけだと明記する

##### 理由
- 今回の修正で、質問1は「3軸物理分割を採るが、同じ target path は共有しない」という条件付き回答に直した。
- 質問11は `ssot-core` 固定の共通ファイルと、schema / policies の専有 path を切り分けた。
- 質問16は「内容相性の確認主体」を Planner に戻し、`ref` 固定は質問18へ分離した。
- これにより、残る論点は path ownership の詳細表や include ルールの最終選択など、DESIGN で詰めるべき項目に絞られる。

### 回答

① 内部矛盾は今回の修正で解消し、残るのは DESIGN で確認すべき論点だけだと明記する

- 「推奨」と「回答」が違う箇所は、回答者が推奨と異なる選択をした結果であり、それ自体は矛盾ではない。
- 一方で、回答の前提条件が不足していた箇所は今回補足した。
- 現時点で残る確認事項は、`catalog-path-ownership-draft.md` の拡張範囲、include 基準の最終選択、DESIGN Issue へどう固定値を落とすかである。
