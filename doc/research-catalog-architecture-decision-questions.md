...
- target repo では `ssot-bot.yml` の `ref` に固定の git ref（tag / branch / commit）を書く。
- `skill-catalog` は DESIGN 時の判断材料として使うが、実行時に整合性を自動判定しない。
- 内容の相性確認は Planner が DESIGN 時に行い、controller は固定された `ref` をそのまま配る。
...
#### 問題
見出し「## 質問16: version が合っていても、中身の相性が悪いときはどうするのか」の回答では、version 管理は不要で、各 repo の `main` ブランチを最新として見る運用が示されている。  
